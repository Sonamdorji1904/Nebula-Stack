...
class CheckinService {
  ...
  async ingestPatientData(metadata = {}) {
    try {
      ...
      // STEP 4: Generate initial DWE token using department counter
      ...
      // NEW: Emit queue update for the department
      try {
        if (queueSocketHandler && queueSocketHandler.emitQueueUpdate) {
          await queueSocketHandler.emitQueueUpdate(department);
        }
      } catch (socketError) {
        logger.warn('Failed to emit queue update after check-in', {
  ...
  async updateTokenStatus(patientId, token, department, newStatus) {
    try {
      ...
      try {
        if (queueSocketHandler && queueSocketHandler.emitTokenStatusChange) {
          await queueSocketHandler.emitTokenStatusChange(
            department,
            token,
            newStatus
          );
        ...
        logger.warn('Failed to emit socket update', {
          token,
          error: socketError.message
        });
      }
  ...
  async addStageToken(patientId, tokenDataOrDepartment) {
    try {
      ...
      try {
        ...
        logger.warn('Failed to emit queue update after adding token', {
          error: socketError.message
        });
      }
